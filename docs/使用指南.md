...
```bash
# 索引文档（将 1 替换为你的文档 ID）
curl -X POST "http://localhost:8001/api/documents/1/index"
```
...
```
curl.exe -X POST "http://localhost:8001/api/documents/upload" -F "file=@data\Linear.md"   
curl.exe -X POST "http://localhost:8001/api/documents/14/index"
Invoke-RestMethod -Uri "http://localhost:8001/api/search/simple" -Method Post -ContentType "application/json" -Body '{"query":"SECOND EDITION","top_k":5,"doc_ids":[14]}'   
```
...
**Agent 智能搜索**（推荐）：
```bash
curl -X POST "http://localhost:8001/api/search/search" \
  -H "Content-Type: application/json" \
  -d '{
    "query": "广西百色重点开发开放试验区建设实施方案",
    "top_k": 5
  }'
```
...
**简单关键词搜索**：
```bash
curl -X POST "http://localhost:8001/api/search/simple" \
  -H "Content-Type: application/json" \
  -d '{
    "query": "机器学习",
    "top_k": 3
...
```
curl -X POST "http://localhost:8001/api/documents/upload" -F "file=@./data/20210309 平果市国土空间规划-初步汇报成果——2.7部分中心城区演变.pdf"
```
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/search/search` | POST | Agent 智能搜索 |
